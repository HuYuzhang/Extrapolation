...
from network.DENET_VERBOSE_FUSE import Network
...
def main(lr, batch_size, epoch, gpu, train_set, valid_set):
    ...
    writer = SummaryWriter(comment="_verbose_fuse_DENET")
	# ------------- Part for tensorboard --------------
    ...
    for epoch in range(0,EPOCH):
        ...
        for imgL, imgR, label in trainLoader:
            ...
            output = SepConvNet(diff, imgL, imgR)
        ...
        torch.save(SepConvNet.state_dict(),
                os.path.join('.', 'verbose_fuse_DENET_iter' + str(epoch + 1)
                                + '-ltype_fSATD_fs'
