...
from network.FKCNN_OPT import Network
...
def main(lr, batch_size, epoch, gpu, train_set, valid_set):
    ...
    writer = SummaryWriter(comment="_opt_FKCNN")
	# ------------- Part for tensorboard --------------
    ...
    for epoch in range(0,EPOCH):
        ...
        for imgL, imgR, label in trainLoader:
            ...
            with torch.no_grad():
                ...
                diff1 = opter.calcOpt(imgR, imgL)
                diff2 = opter.calcOpt(imgL, imgR)

            output = SepConvNet(diff1, diff2, imgL, imgR)
        ...
        torch.save(SepConvNet.state_dict(),
                os.path.join('.', 'OPT_FKCNN_iter' + str(epoch + 1)
                                + '-ltype_fSATD_fs'
