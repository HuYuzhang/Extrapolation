...
class Opter():
    def __init__(self, height, width, batch_size):
        ...
        tensorHorizontal = torch.linspace(-1.0, 1.0, width).view(1, 1, 1, width).expand(batch_size, -1, height, -1)
        tensorVertical = torch.linspace(-1.0, 1.0, height).view(1, 1, height, 1).expand(batch_size, -1, -1, width)
        self.tensorAxisMap = torch.cat([ tensorHorizontal, tensorVertical ], 1).cuda()
