...
class mydataset(data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        img1 = (cv2.cvtColor(img2, cv2.COLOR_RGB2GRAY).astype(np.float32) - cv2.cvtColor(img1, cv2.COLOR_RGB2GRAY).astype(np.float32)).reshape([img2.shape[0], img2.shape[1], 1])
        # IPython.embed()
        # exit()
