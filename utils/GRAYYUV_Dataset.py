...
class mydataset(data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        img1 = cv2.cvtColor(img1, cv2.COLOR_BGR2YUV).astype(np.float32) / 255.0
        img2 = cv2.cvtColor(img2, cv2.COLOR_BGR2YUV).astype(np.float32) / 255.0
        img3 = cv2.cvtColor(img3, cv2.COLOR_BGR2YUV).astype(np.float32) / 255.0
        diff = img2[:,:,0:1] - img1[:,:,0:1]
        # IPython.embed()
        # exit()
        ...
        return var(torch.from_numpy(diff.transpose(2,0,1))), var(torch.from_numpy(img2.transpose(2,0,1))), \
               var(torch.from_numpy(img3.transpose(2,0,1)))