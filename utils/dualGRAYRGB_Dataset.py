...
class mydataset(data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        diff = (cv2.cvtColor(img2, cv2.COLOR_RGB2GRAY).astype(np.float32) - cv2.cvtColor(img1, cv2.COLOR_RGB2GRAY).astype(np.float32)).reshape([img2.shape[0], img2.shape[1], 1])
        # IPython.embed()
        # exit()
        ...
        return var(torch.from_numpy(diff.transpose(2,0,1).astype(np.float32) / 255.0)), var(torch.from_numpy(img1.transpose(2,0,1).astype(np.float32) / 255.0)), var(torch.from_numpy(img2.transpose(2,0,1).astype(np.float32) / 255.0)), \
