...
from network.CZAR_FUSE import Network
...
class PWCNet(torch.nn.Module):
	def __init__(self):
		...
		self.load_state_dict(torch.load('/mnt/hdd/iku/ISCAS/utils/network-default.pytorch'))
	# end
...
def main(lr, batch_size, epoch, gpu, train_set, valid_set):
    ...
    writer = SummaryWriter(comment="_fuse_CZAR")
	# ------------- Part for tensorboard --------------
    ...
    for epoch in range(0,EPOCH):
        ...
        for imgL, imgR, label in trainLoader:
            ...
            conved, warped, output = SepConvNet(diff, imgL, imgR)
            loss_conv = SepConvNet_cost(conved, label)
            loss_out = SepConvNet_cost(output, label)
            loss_warp = SepConvNet_cost(warped, label)
            loss = 0.5 * loss_out + 0.3 * loss_warp + 0.2 * loss_conv
            loss.backward()
            ...
            if cnt % printinterval == 0:
                ...
                writer.add_image("Conved image", conved[0], cnt)
                writer.add_image("Ref image", imgR[0], cnt)

                writer.add_scalar('Train Batch SATD loss', loss_out.data.item(), int(global_step / printinterval))
                ...
                print('Epoch [%d/%d], Iter [%d/%d], Time [%4.4f], Batch loss [%.6f], Interval loss [%.6f]' %
                    (epoch + 1, EPOCH, cnt, len(trainset) // BATCH_SIZE, time.time() - start_time, loss_out.data.item(), tsumloss / printinterval))
                tsumloss = 0.0
        ...
        torch.save(SepConvNet.state_dict(),
                os.path.join('.', 'fuse_CZAR_iter' + str(epoch + 1)
                                + '-ltype_fSATD_fs'
