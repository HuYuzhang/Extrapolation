...
I begin this version this afternoon(8.23)
In fact, this version will do no help to the prediciton, becauseI froze all the forward-prediction weights

So the target is only to improve the performance of the backward prediction
But how to froze some layer and how to apply the trained-weights can be learned here
'''
import sys
import torch
...
class Network(torch.nn.Module):
    ...
    # ------------------- LSTM Part -----------------
        ...
        self.moduleLSTM_b = ConvLSTM(3, 32)
        self.moduleConvH_b = Basic(32, 32)
    ...
    # ------------------- LSTM Part --------------------
        if tensorResidual is None:
            ...
            if direction == 0:
                tensorH_next, tensorC_next = self.moduleLSTM(tensorResidual)
            else:
                tensorH_next, tensorC_next = self.moduleLSTM_b(tensorResidual)
        else:
            if direction == 0:
                tensorH_next, tensorC_next = self.moduleLSTM(tensorResidual, tensorHidden)
            else:
                tensorH_next, tensorC_next = self.moduleLSTM_b(tensorResidual, tensorHidden)
    ...
    for epoch in range(0,EPOCH):
        ...
        for label_list in trainLoader:
            ...
            for i in range(5):
                ...
                if i == 0:
                    output_f, stat_f = SepConvNet(imgL, imgR, 0)
                else:
                    output_f, stat_f = SepConvNet(imgL, imgR, 0, res_f, stat_f)
                ...
                SepConvNet_optimizer.zero_grad()
                if i == 0:
                    output_b, stat_b = SepConvNet(output_f, imgR, 1)
                else:
                    output_b, stat_b = SepConvNet(output_f, imgR, 1, res_b, stat_b)
        ...
        for label_list in valLoader:
            ...
            with torch.no_grad():
                for i in range(5):
                    ...
                    loss = SepConvNet_cost(output_b, label_L)
                    psnr = psnr + calcPSNR.calcPSNR(output_b.cpu().data.numpy(), label_L.cpu().data.numpy())
                    sumloss = sumloss + loss.data.item()
                    # sumloss_b = sumloss_b + loss.data.item()
                    # tsumloss_b = tsumloss_b + loss.data.item()
        ...
        torch.save(SepConvNet.state_dict(),
                os.path.join('.', 'tail_dual_LSTM_iter' + str(epoch + 1)
                                + '-ltype_fSATD_fs'
