...
def main(lr, batch_size, epoch, gpu, train_set, valid_set):
    ...
    SepConvNet.load_state_dict(torch.load('dual_beta_LSTM_iter4-ltype_fSATD_fs-lr_0.001-trainloss_1.1409-evalloss_0.1201-evalpsnr_29.4596.pkl'))
    ...
    for epoch in range(0,EPOCH):
        ...
        for label_list in trainLoader:
            ...
            # Then begin the backward training~
            bad_list.reverse()
            label_list.reverse()
            for i in range(5):
                imgL = var(bad_list[i]).cuda()
            ...
            if cnt % printinterval == 0:
                print('Epoch [%d/%d], Iter [%d/%d], Time [%4.4f], Batch loss [%.6f], Interval loss [%.6f]' %
                    (epoch + 1, EPOCH, cnt, len(trainset) // BATCH_SIZE, time.time() - start_time, loss.data.item(), tsumloss / printinterval / 10))
                tsumloss = 0.0
        print('Epoch [%d/%d], iter: %d, Time [%4.4f], Avg Loss [%.6f]' %
            (epoch + 1, EPOCH, cnt, time.time() - start_time, sumloss / cnt / 10))
        ...
        torch.save(SepConvNet.state_dict(),
                os.path.join('.', 'dual_beta_LSTM_iter' + str(epoch + 1)
                                + '-ltype_fSATD_fs'
