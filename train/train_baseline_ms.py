'''
This version is the raw implementation of FKCNN with multi-scale, seems heigher PSNR but lower BD-RATE?
'''
import sys
import torch
...
def main(lr, batch_size, epoch, gpu, train_set, valid_set):
    ...
    writer = SummaryWriter(log_dir='tb/baseline_ms')
    # ------------- Part for tensorboard --------------
    ...
    for epoch in range(0,EPOCH):
        ...
        torch.save(SepConvNet.state_dict(),
                os.path.join('.', 'ms_baseline_iter' + str(epoch + 1)
                                + '-ltype_fSATD_fs'
...
if __name__ == "__main__":
    ...
    parser.add_argument("--gpu", type=int, dest="gpu", required=True,
                        help="GPU device id")
    parser.add_argument("--train_set", type=str, dest="train_set", default="/mnt/hdd/xiasifeng/vimeoimgs_LD_crop", 
                        help="Path of the training set")
    parser.add_argument("--valid_set", type=str, dest="valid_set", default="/mnt/hdd/xiasifeng/vimeoimgs_LD_val_crop", 
