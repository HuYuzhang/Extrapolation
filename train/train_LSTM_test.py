...
The structure is based on tail so different from the previous ones~

This version is my pride, because I freeze the weights which belong to the baseline. By this way, I can promise that the encoder-decoder just do the right thing: describe the motion state.

What's more, by this way, less param need to be updated, which results in a faster learning speed.

Now under training... Though the begining seems worse... but I believe in it.
'''
import sys
import torch
