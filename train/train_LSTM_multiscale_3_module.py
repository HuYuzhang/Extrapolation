...
The structure is based on tail2... But it seems that I should use tail?

I just set 3 different convLSTM module for different resolutions. Which means that I have 3-state each time... so the param list is quite long hhh
...
class Network(torch.nn.Module):
    def __init__(self):
        ...
        self.mv1_a_ = Subnet(288,13)
        self.mv2_a_ = Subnet(288, 13)
        self.mh1_a_ = Subnet(288, 13)
        self.mh2_a_ = Subnet(288, 13)
        ...
        self.mv1_b_ = Subnet(160, 25)
        self.mv2_b_ = Subnet(160, 25)
        self.mh1_b_ = Subnet(160, 25)
        self.mh2_b_ = Subnet(160, 25)
        ...
        # ------------------- LSTM Part -----------------
        self.moduleLSTM0 = ConvLSTM(32, 32)
        self.moduleLSTM1 = ConvLSTM(32, 32)
        self.moduleLSTM2 = ConvLSTM(32, 32)
        self.moduleConvH0 = Basic(3, 32)
        self.moduleConvH1 = Basic(3, 32)
        self.moduleConvH2 = Basic(3, 32)
        self.moduleDownH = torch.nn.AvgPool2d(kernel_size=2, stride=2)
        # self.moduleDownLSTM1 = torch.nn.Sequential(
        #         torch.nn.Conv2d(in_channels=32, out_channels=64, kernel_size=2, stride=2, padding=0),
        #         torch.nn.ReLU(inplace=False))
        # self.moduleDownLSTM2 = torch.nn.Sequential(
        #         torch.nn.Conv2d(in_channels=64, out_channels=128, kernel_size=2, stride=2, padding=0),
        #         torch.nn.ReLU(inplace=False))
        # ------------------- Initialize Part -----------------
    ...
    def forward(self, tensorInput1, tensorInput2, tensorResidual=None, tensorHidden0=None, tensorHidden1=None, tensorHidden2=None):
        '''
    ...
    # ------------------- LSTM Part --------------------
        if tensorResidual is None:
            tensorResidual0 = var(torch.zeros(batch_size, tensorInput1.size(1), tensorInput1.size(2), tensorInput1.size(3))).cuda()
            tensorResidual1 = var(torch.zeros(batch_size, tensorInput1.size(1), tensorInput1.size(2) // 2, tensorInput1.size(3) // 2)).cuda()
            tensorResidual2 = var(torch.zeros(batch_size, tensorInput1.size(1), tensorInput1.size(2) // 4, tensorInput1.size(3) // 4)).cuda()

            tensorResidual0 = self.moduleDownH(self.moduleConvH0(tensorResidual0))
            tensorResidual1 = self.moduleDownH(self.moduleConvH1(tensorResidual1))
            tensorResidual2 = self.moduleDownH(self.moduleConvH2(tensorResidual2))

            tensorH_next0, tensorC_next0 = self.moduleLSTM0(tensorResidual0) 
            tensorH_next1, tensorC_next1 = self.moduleLSTM1(tensorResidual1) 
            tensorH_next2, tensorC_next2 = self.moduleLSTM2(tensorResidual2) 
        else:
            tensorResidual0 = tensorResidual
            tensorResidual1 = func.interpolate(tensorResidual, scale_factor=0.5,  mode='bilinear',align_corners=True)
            tensorResidual2 = func.interpolate(tensorResidual, scale_factor=0.25, mode='bilinear',align_corners=True)

            tensorResidual0 = self.moduleDownH(self.moduleConvH0(tensorResidual0))
            tensorResidual1 = self.moduleDownH(self.moduleConvH1(tensorResidual1))
            tensorResidual2 = self.moduleDownH(self.moduleConvH2(tensorResidual2))
            
            tensorH_next0, tensorC_next0 = self.moduleLSTM0(tensorResidual0, tensorHidden0) 
            tensorH_next1, tensorC_next1 = self.moduleLSTM1(tensorResidual1, tensorHidden1) 
            tensorH_next2, tensorC_next2 = self.moduleLSTM2(tensorResidual2, tensorHidden2) 
        ...
        tensorL0 = tensorH_next0
        tensorL1 = tensorH_next1
        tensorL2 = tensorH_next2
        ...
        return tensorDot1 + tensorDot2, tensorDot1_a + tensorDot2_a, tensorDot1_b + tensorDot2_b, (tensorH_next0, tensorC_next0), (tensorH_next1, tensorC_next1), (tensorH_next2, tensorC_next2)
...
def main(lr, batch_size, epoch, gpu, train_set, valid_set):
    ...
    for epoch in range(0,EPOCH):
        ...
        for label_list in trainLoader:
            ...
            for i in range(5):
                ...
                if i == 0:
                    ...
                    output, output_a, output_b, stat0, stat1, stat2 = SepConvNet(imgL, imgR)
                    ...
                    output, output_a, output_b, stat0, stat1, stat2 = SepConvNet(imgL, imgR, res, stat0, stat1, stat2)
                    res = poor_label - output
                    # loss = SepConvNet_cost(output, label)
                    loss = 0.5*SepConvNet_cost(output, label) + \
                            0.2*SepConvNet_cost(output_a,func.upsample(label, size=(label.shape[2] // 4, label.shape[3] // 4), mode='bilinear',align_corners=True)) + \
        ...
        torch.save(SepConvNet.state_dict(),
                os.path.join('.', '3module_multiscale_bicubic_LSTM_iter' + str(epoch + 1)
                                + '-ltype_fSATD_fs'
