...
Forward and backward has their own LSTM module. 
...
class Network(torch.nn.Module):
    ...
    # ------------------- LSTM Part -----------------
        self.moduleLSTM = ConvLSTM(3, 32) # child 25 
...
def main(lr, batch_size, epoch, gpu, train_set, valid_set):
    ...
    grad_list = [18,19,20,21,  25,26,27,28,29,30]
    child_cnt = 0
    ...
    for epoch in range(0,EPOCH):
        ...
        torch.save(SepConvNet.state_dict(),
                os.path.join('.', 'test_tail_dual_LSTM_iter' + str(epoch + 1)
                                + '-ltype_fSATD_fs'
