...
For the downsample operation, I just use the bicubic operation, would it be useful?
...
class Network(torch.nn.Module):
    ...
    # ------------------- LSTM Part --------------------
        ...
        tensorL1 = func.interpolate(tensorH_next, scale_factor=0.5, mode='bilinear',align_corners=True)
        tensorL2 = func.interpolate(tensorH_next, scale_factor=0.25, mode='bilinear',align_corners=True)
...
def main(lr, batch_size, epoch, gpu, train_set, valid_set):
    ...
    for epoch in range(0,EPOCH):
        ...
        torch.save(SepConvNet.state_dict(),
                os.path.join('.', 'multiscale_bicubic_LSTM_iter' + str(epoch + 1)
                                + '-ltype_fSATD_fs'
